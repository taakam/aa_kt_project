...
OOCYTE_RADIUS = 0.010          # outer ZP radius; preserve old policy geometry
INNER_RADIUS = 0.0078            # inner volume-deformable oocyte/ooplasm proxy
ZP_SURFACE_THICKNESS = 0.0010

# Outer zona pellucida mechanics.
ZP_YOUNGS_MODULUS = 7_340.0
ZP_POISSON_RATIO = 0.04
ZP_DENSITY = 1000.0

# Inner oocyte/ooplasm mechanics: provisional test values.
INNER_YOUNGS_MODULUS = 1_000.0
INNER_POISSON_RATIO = 0.45
...
class MecaReachEnvCfg(DirectRLEnvCfg):
    ...
    oocyte_cfg: DeformableObjectCfg = DeformableObjectCfg(
        prim_path="/World/envs/env_.*/InnerOocyte",
        spawn=sim_utils.MeshSphereCfg(
            ...
            visual_material=sim_utils.PreviewSurfaceCfg(
                diffuse_color=(1.0, 0.55, 0.35),
                opacity=0.80,
            ),
            physics_material=sim_utils.DeformableBodyMaterialCfg(
                density=INNER_DENSITY,
                youngs_modulus=INNER_YOUNGS_MODULUS,
                poissons_ratio=INNER_POISSON_RATIO,
...
class MecaReachEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MecaReachEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print(
            f"[reach-env] layered oocyte: ZP R={OOCYTE_RADIUS:.3f} m, "
            f"ZP E={ZP_YOUNGS_MODULUS/1000:.2f} kPa, "
            f"inner R={INNER_RADIUS:.4f} m, inner E={INNER_YOUNGS_MODULUS/1000:.2f} kPa",
            flush=True,
        )
        print(
    ...
    @staticmethod
    def _create_uv_sphere_mesh(stage, path: str, radius: float):
        """Create a closed triangle sphere centered at the prim origin."""
        import math
        lat_segments, lon_segments = 18, 28
        mesh = UsdGeom.Mesh.Define(stage, path)
        pts = [Gf.Vec3f(0.0, 0.0, radius)]
        for i in range(1, lat_segments):
            th = math.pi * i / lat_segments
            for j in range(lon_segments):
                ph = 2.0 * math.pi * j / lon_segments
                pts.append(Gf.Vec3f(
        ...
        south = len(pts)
        pts.append(Gf.Vec3f(0.0, 0.0, -radius))
        faces = []
        for j in range(lon_segments):
            faces.append((0, 1 + j, 1 + (j + 1) % lon_segments))
        for i in range(lat_segments - 2):
            r0 = 1 + i * lon_segments
            r1 = r0 + lon_segments
            for j in range(lon_segments):
                n = (j + 1) % lon_segments
                faces += [(r0+j, r1+j, r0+n), (r0+n, r1+j, r1+n)]
        last = 1 + (lat_segments - 2) * lon_segments
        for j in range(lon_segments):
            faces.append((last+j, south, last+(j+1) % lon_segments))
        mesh.GetPointsAttr().Set(pts)
        mesh.GetFaceVertexCountsAttr().Set([3] * len(faces))
        mesh.GetFaceVertexIndicesAttr().Set([v for f in faces for v in f])
        mesh.GetSubdivisionSchemeAttr().Set("none")
        return mesh

    def _spawn_zp_surface(self, env_path: str) -> None:
        """Spawn the outer ZP as a low-level PhysX surface deformable."""
        stage = omni.usd.get_context().get_stage()
        path = f"{env_path}/ZonaPellucida"
        mesh = self._create_uv_sphere_mesh(stage, path, OOCYTE_RADIUS)
        ok = deformableUtils.set_physics_surface_deformable_body(
            stage, mesh.GetPrim().GetPath()
        )
        if not ok:
            raise RuntimeError(f"Failed to create surface-deformable ZP at {path}")
        ...
        # Physics material using the OmniPhysics schema names available in Sim 5.1.
        mat = UsdShade.Material.Define(stage, f"{env_path}/ZP_PhysicsMaterial")
        mp = mat.GetPrim()
        mp.ApplyAPI("OmniPhysicsBaseMaterialAPI")
        mp.GetAttribute("omniphysics:density").Set(ZP_DENSITY)
        mp.GetAttribute("omniphysics:dynamicFriction").Set(0.2)
        mp.ApplyAPI("OmniPhysicsDeformableMaterialAPI")
        mp.GetAttribute("omniphysics:youngsModulus").Set(ZP_YOUNGS_MODULUS)
        mp.GetAttribute("omniphysics:poissonsRatio").Set(ZP_POISSON_RATIO)
        mp.ApplyAPI("OmniPhysicsSurfaceDeformableMaterialAPI")
        mp.GetAttribute("omniphysics:surfaceThickness").Set(ZP_SURFACE_THICKNESS)
        bend = ZP_YOUNGS_MODULUS / (12.0 * (1.0 - ZP_POISSON_RATIO**2))
        mp.GetAttribute("omniphysics:surfaceBendStiffness").Set(bend)
        UsdShade.MaterialBindingAPI.Apply(prim).Bind(
            mat, UsdShade.Tokens.weakerThanDescendants, "physics"
        )

        # Bright orange-red visual: intentionally compatible with the old
        # r > 1.35*g and r > 1.35*b wrist-camera segmentation.
        vmat = UsdShade.Material.Define(stage, f"{env_path}/ZP_VisualMaterial")
        shader = UsdShade.Shader.Define(stage, f"{env_path}/ZP_VisualMaterial/Shader")
        shader.CreateIdAttr("UsdPreviewSurface")
        shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(
            Gf.Vec3f(1.0, 0.20, 0.05)
        )
        shader.CreateInput("opacity", Sdf.ValueTypeNames.Float).Set(0.45)
        vmat.CreateSurfaceOutput().ConnectToSource(shader.ConnectableAPI(), "surface")
        UsdShade.MaterialBindingAPI.Apply(prim).Bind(vmat)

    def _update_zp_pose(self) -> None:
        """Move each ZP prim to the same randomized center as the inner body."""
        stage = omni.usd.get_context().get_stage()
        for i in range(self.num_envs):
            prim = stage.GetPrimAtPath(f"/World/envs/env_{i}/ZonaPellucida")
            if not prim.IsValid():
                continue
            ...
            # _target_pos_w is world-space; env prim is already under env origin,
            # so use target position relative to that environment.
            local = self._target_pos_w[i] - self.scene.env_origins[i]
            translate.Set(Gf.Vec3d(*[float(v) for v in local.tolist()]))
    ...
    def _setup_scene(self) -> None:
        ...
        # Low-level surface deformable must exist in env_0 before cloning.
        self._spawn_zp_surface("/World/envs/env_0")
    ...
    def _debug_print_oocyte_motion(self) -> None:
        ...
        x_extent_fraction = extents[:, 0] / (2.0 * INNER_RADIUS)
        y_extent_fraction = extents[:, 1] / (2.0 * INNER_RADIUS)
        z_extent_fraction = extents[:, 2] / (2.0 * INNER_RADIUS)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | torch.Tensor | None) -> None:
        ...
        # The 10-mm outer ZP proxy sits on the dish; target position stores its center.
